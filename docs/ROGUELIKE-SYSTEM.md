...
├── ShockwaveEmitter.cs           # MonoBehaviour: aura VFX + ring author sẵn + tick damage toả tròn từ chân player
...
| `ShockwaveEmitter` | `SetAura`/`Deactivate` bật-tắt và scale prefab lồng `Vfx_ShockwaveAura` (hình của aura, chạy loop suốt thời gian sở hữu skill); `Pulse` overlap quanh chân player mỗi nhịp, damage đồng nhất khắp bán kính + lực dưới ngưỡng physics (hích, không hất bay), ring scale ease-out bằng timer mỗi tick | Không còn impulse camera lẫn khói burst: mỗi giây một lần thì cả hai thành nhiễu. Tiếng giữ nhịp riêng `_soundInterval` 2 s và chỉ kêu khi tick thật sự trúng ai đó |
...
- Nhánh `Player/Shockwave` (local 0): `ShockwaveEmitter` với `_zombies`, `_audio`, `_enemyMask`, `_clip` = `AssaultCanon_1p_tail`, `_soundInterval` 2, cùng hai lớp hình là hai con **author sẵn inactive**:
  ...
  - `_ring` = con **`Ring`** (mesh `Models/VFX/ShockwaveAnnulus.asset`, material `Art/Materials/VFX/ShockwaveRing.mat`, không collider) + `_ringRenderer`, `_ringColorOverLifetime` (gradient ghi qua MPB `_BaseColor`), `_ringDuration` 0.48 — vòng toả ra mỗi nhịp để báo cú tick vừa ăn damage.
...
**`Vfx_ShockwaveAura.prefab`** (thêm 2026-09-07 theo yêu cầu user: "skill shockwave hãy dùng VFX trong Epic Toon FX/Prefabs/Combat/Magic/Aura") dựng từ **`MagicAuraBlue`** của pack — chọn bản Blue vì `_accentColor` của skill là tím nhạt (0.79, 0.72, 1), gần Blue hơn Green/Yellow. Quy trình: `PrefabUtility.LoadPrefabContents` bản của pack → đổi tên → `SaveAsPrefabAsset` sang `_ZombieWar/Prefabs/VFX/`, **không** kèm `PooledVfx` vì đây không phải VFX bắn qua `VfxService` mà là một node thường trực. Cấu trúc giữ nguyên của pack: root (particle nền, material `slash_circle01_ADD`) + `MagicSparkles` (`magic_orb2_ADD`, simulation space **World** nên tia sáng rớt lại phía sau khi player chạy — đúng chất aura kéo đuôi) + `Waves` (`aura_vertical_ADD`, render mode Mesh với mesh `aura_straight.FBX`). Cả ba `looping` + `playOnAwake` nên bật GameObject là chạy, tắt là hết — không cần code Play/Stop. `scalingMode: Hierarchy` là điều kiện để `SetAura` scale được cả effect bằng transform. 7 asset đã move từ `ThirdParty/Epic Toon FX` sang `_ZombieWar` bằng `AssetDatabase.MoveAsset` (giữ GUID): 3 material vào `Art/Materials/VFX/`, 3 texture vào `Art/Textures/VFX/`, `aura_straight.FBX` vào `Models/VFX/`. **Không** cần move `Models/Materials/verticalglow02.mat` — `.meta` của FBX không có `externalObjects` remap, nó chỉ tìm material theo tên (`materialSearch: 1`), mà game chỉ dùng *mesh* của FBX (material do ParticleSystemRenderer cấp), nên không có tham chiếu xuyên `ThirdParty/` nào còn lại.
