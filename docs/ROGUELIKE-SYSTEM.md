...
├── ShockwaveSkillSO.cs           # active aura: cooldown chính là nhịp tick 1 s → ShockwaveEmitter.Pulse()
...
├── ShockwaveEmitter.cs           # MonoBehaviour: bật/scale aura VFX + tick damage toả tròn từ chân player
...
└── Gun_Drone.asset               # GunDefinitionSO riêng của drone: 5.5 dmg / 0.25 s, không nằm trong shop
...
| `ShockwaveEmitter` | `SetAura`/`Deactivate` bật-tắt và scale prefab lồng `Vfx_ShockwaveAura` (hình của aura, chạy loop suốt thời gian sở hữu skill); `Pulse` overlap quanh chân player mỗi nhịp, damage đồng nhất khắp bán kính + lực dưới ngưỡng physics (hích, không hất bay) | **Nhịp đập không có hình riêng** (bỏ 2026-09-08 theo yêu cầu user): impulse camera, khói burst rồi tới vòng ring annulus đều đã gỡ — mỗi giây một lần thì thứ nào cũng thành strobe đè lên chính cái aura vốn đã vẽ sẵn bán kính. Chỉ còn tiếng, giữ nhịp riêng `_soundInterval` 2 s và chỉ kêu khi tick thật sự trúng ai đó |
...
|---|---|---|---|---|
| **AUTO GRENADE** | Cứ hết cooldown là ném một **volley** về phía mục tiêu auto-aim (không có thì theo hướng chạy). Volley = `1 + ExtraBombsPerVolley` quả, quả thêm rơi lệch ngẫu nhiên trong `VolleyScatter` 1.6 m. Sức nổ lấy từ `BombDefinition`: 105 dmg ở tâm → 40 dmg ở mép, bán kính 4.5 m | cooldown 4.5 s × 0.84ⁿ, sàn 1.5 s | 5 | `Item_Icon_Grenade` |
| **ORBIT BLADES** | Lưỡi cưa xoay quanh người 240°/s ở bán kính 2.2 m; mỗi lưỡi quét overlap, một zombie ăn tick tối đa ~3.6 lần/giây (`_hitInterval` 0.28) | 2 lưỡi + 1/cấp (trần 6 = số lưỡi author trong prefab), damage 9.5 × 1.2ⁿ | 5 | `Icon_Change` |
| **MOLOTOV** | Ném chai theo cung y hệt grenade (tầm 6 m, bay 0.6 s), vỡ thành vùng lửa bán kính 2.2 m đốt zombie đứng trong đó 5.5 dmg mỗi 0.4 s (~13.8 dps, Walker 24 HP chết sau ~1.8 s). Mỗi tick lửa là một `HitStun` 0.14 s nên lửa còn **làm chậm** kẻ đi qua. Không đốt player | cooldown 5.5 s × 0.85ⁿ sàn 2.5 s, cháy 4 s + 1 s/cấp, damage × 1.15ⁿ | 5 | `Item_Icon_Burning_Bottle` |
| **CHAIN LIGHTNING** | Chỉ bắn khi auto-aim có mục tiêu (`CanTrigger`). Sét đánh mục tiêu rồi nhảy sang con gần nhất chưa trúng trong 4 m, mỗi lần nhảy damage × 0.8, không cần đường bắn thẳng nên với được phía sau lưng. 26 dmg đầu, 3 lần nhảy | cooldown 3.2 s × 0.88ⁿ sàn 1.6 s, +1 lần nhảy/cấp (trần 7 = 8 bolt author sẵn) | 5 | `Item_Icon_Car_Battery` |
| **SHOCKWAVE** | **Aura**, không còn là cú nổ chờ thời cơ (đổi 2026-09-07 theo yêu cầu user): cứ **mỗi giây** một nhịp gõ mọi zombie trong 3.5 m quanh chân player 7 dmg + lực 1.5 (dưới ngưỡng physics 4 nên hích chứ không hất bay). Không có `CanTrigger`, không đợi đủ đám đông — cooldown của skill **chính là** nhịp tick. Hai lớp hình: prefab `Vfx_ShockwaveAura` (Epic Toon FX) cháy liên tục và scale đúng bằng bán kính sát thương, cộng ring toả ra mỗi nhịp để báo cú tick vừa ăn damage | nhịp giữ 1 s ở mọi cấp; bán kính +0.3 m/cấp, damage × 1.2ⁿ | 5 | `Item_Icon_Megaphone` |
| **SENTRY DRONE** | Liên tục. Drone **bay vòng quanh player** trong world space (bán kính 1.4 m, 60°/s, cao 1.7 m, bob 0.12 m / 1.2 Hz, SmoothDamp 0.2 s bám theo) nên luôn di chuyển và không xoay theo nhân vật; quét 10 m mỗi 0.2 s, ưu tiên con **khác** mục tiêu của player (phạt 4 m), bắn qua pipeline đạn chung với `Gun_Drone` 5.5 dmg / 0.25 s (22 dps, ≈ 37% rifle nền 60 dps; 5 viên hạ Walker 24 HP, 3 viên hạ Runner 14 HP) | damage × 1.2ⁿ, nhịp bắn × 0.9ⁿ | 5 | `Item_Icon_Machine` |

Knockback của lưỡi (1.2), sét (1.0) và aura shockwave (1.5) cố ý dưới `PhysicsKnockbackThreshold` (4) của zombie: chúng **hích**, không hất bay — hất bay liên tục sẽ phá nhịp chase. Sau đợt 2026-09-07 thì **grenade là active duy nhất còn vượt ngưỡng đó**.

**Cân bằng lại toàn bộ active — 2026-09-07 theo yêu cầu user**: giảm cooldown *và* giảm damage cho cả 6 skill. Ý đồ là đổi nhịp chứ không đổi sức mạnh tổng — đòn nhỏ hơn nhưng dày hơn, nên màn hình phản hồi liên tục thay vì im lặng rồi bùng một phát; DPS ổn định sau đợt chỉnh gần như y hệt trước. Với hai active liên tục (ORBIT BLADES, SENTRY DRONE) thì "cooldown" tương đương là `_hitInterval` và `_fireInterval` của `Gun_Drone`.

| Skill | Nhịp (trước → sau) | Damage (trước → sau) | DPS ổn định |
|---|---|---|---|
| AUTO GRENADE | 7 s → **4.5 s** (sàn 2 → **1.5**) | tâm 160 → **105**, mép 60 → **40** (`BombDefinition`) | 22.9 → 23.3 |
| ORBIT BLADES | tick 0.4 s → **0.28 s** | 14 → **9.5** | 35 → 33.9 (một lưỡi bám thân) |
| MOLOTOV | 9 s → **5.5 s** (sàn 4 → **2.5**) | 8 → **5.5** mỗi tick 0.4 s | 20 → 13.8 (trong vùng lửa) |
| CHAIN LIGHTNING | 5 s → **3.2 s** (sàn 2.5 → **1.6**) | 40 → **26** | 8 → 8.1 (mục tiêu đầu) |
| SHOCKWAVE | 8 s → **1 s** (thành aura) | 30 mỗi lần nổ → **7** mỗi tick | 3.75 → 7 |
| SENTRY DRONE | 0.35 s → **0.25 s** | 8 → **5.5** (`Gun_Drone`) | 22.9 → 22 |

SHOCKWAVE là ngoại lệ duy nhất được **tăng** DPS ổn định: đổi từ cú nổ chờ thời cơ sang aura thì nó mất hẳn vai trò "gỡ vây" (lực 7 hất bay cả đám) nên phải bù bằng sát thương đều tay. 7 dmg/s trong 3.5 m nghĩa là Walker (24 HP) đứng trong aura 3.4 s là chết, Runner (14 HP) 2 s; ở cấp 5 là 14.5 dmg/s trong 4.7 m. Mỗi tick vẫn đẩy zombie vào `HitStun` 0.14 s (14% thời gian) nên aura còn làm chậm nhẹ đám đông đang ép sát.

**Vì sao có `CanTrigger`**: runner cũ reset cooldown ngay khi về 0 rồi gọi `Trigger`. Với grenade thì ổn (không có mục tiêu vẫn ném về hướng chạy), nhưng sét vào khoảng không là đốt trọn một cooldown. Giờ runner hỏi `CanTrigger` mỗi frame sau khi cooldown về 0 và **giữ cooldown ở 0** cho tới khi skill đồng ý. Các skill còn lại không sửa gì vì mặc định trả `true` — SHOCKWAVE cũng dùng nó tới 2026-09-07, khi nó thành aura thì bỏ luôn (`_minZombiesToTrigger` không còn).
...
- Nhánh `Player/Shockwave` (local 0): `ShockwaveEmitter` với `_zombies`, `_audio`, `_enemyMask`, `_clip` = `AssaultCanon_1p_tail`, `_soundInterval` 2, cùng hai lớp hình là hai con **author sẵn inactive**:
  - `_aura` = con **`Aura`** — instance prefab lồng `Prefabs/VFX/Vfx_ShockwaveAura.prefab`, local 0, scale 1 lúc author. Đây là *hình của aura*: nó bật suốt thời gian sở hữu skill chứ không chớp theo nhịp. `_auraVisualRadius` 3 là bán kính đĩa sáng của effect ở scale 1 (đo bằng `startSize` 6 của particle nền), nên `SetAura(radius)` chỉ việc scale `radius / 3` — vòng sáng luôn trùng đúng vòng ăn damage, kể cả khi lên cấp nới bán kính. Đổi effect khác thì đo lại số này, đừng hard-code scale.
  - ~~`_ring`~~ — **đã gỡ 2026-09-08 theo yêu cầu user** ("bỏ viền xanh mỗi 1s lại hiện ra đi, chỉ cần giữ lại vfx aura thôi"). Node con `Ring` bị xoá khỏi `Player/Shockwave` và cả bốn field ring (`_ring`, `_ringDuration`, `_ringRenderer`, `_ringColorOverLifetime`) cùng `Update`/`OnDisable` bị gỡ khỏi `ShockwaveEmitter`. Hai asset của nó, `Models/VFX/ShockwaveAnnulus.asset` + `Art/Materials/VFX/ShockwaveRing.mat`, **giữ lại nhưng giờ mồ côi** — đã commit trong `8b01155d` nên dựng lại được bất cứ lúc nào; dọn chúng ở đợt quét asset mồ côi kế tiếp nếu không dùng tới.

  Đợt aura 2026-09-07 gỡ `_vfx`, `_burstVfx`, `_impulseSource`, `_cameraImpulse` khỏi script: rung camera và khói burst mỗi giây là quá nhiều.

**`Vfx_ShockwaveAura.prefab`** (thêm 2026-09-07 theo yêu cầu user: "skill shockwave hãy dùng VFX trong Epic Toon FX/Prefabs/Combat/Magic/Aura") dựng từ **`MagicAuraBlue`** của pack — chọn bản Blue vì `_accentColor` của skill là tím nhạt (0.79, 0.72, 1), gần Blue hơn Green/Yellow. Quy trình: `PrefabUtility.LoadPrefabContents` bản của pack → đổi tên → `SaveAsPrefabAsset` sang `_ZombieWar/Prefabs/VFX/`, **không** kèm `PooledVfx` vì đây không phải VFX bắn qua `VfxService` mà là một node thường trực. Cấu trúc **đã tỉa còn 2 system** (user chọn 2026-09-08 sau khi xem bản đầy đủ trong game): root (đĩa nền, mesh Quad `startSize` 6 → bán kính 3, material `slash_circle01_ADD`) + `MagicSparkles` (`magic_orb2_ADD`, simulation space **World** nên tia sáng rớt lại phía sau khi player chạy — đúng chất aura kéo đuôi). System thứ ba của pack, **`Waves`** (`aura_vertical_ADD` + mesh `aura_straight.FBX`), **đã xoá**: nó là một đĩa đặc bán kính ~2.6 quay tròn, chồng lên đĩa nền thành một mảng cyan dày lấp kín vùng chơi — nhìn top-down thì che mất zombie và mặt đất. Cùng lý do đó `startColor` của đĩa nền hạ từ trắng đặc xuống **RGBA(0.4, 0.4, 0.4, 0.55)**: material là additive nên phải kéo cả **RGB** xuống, không chỉ alpha — nhiều shader particle additive bỏ qua alpha. Cả hai `looping` + `playOnAwake` nên bật GameObject là chạy, tắt là hết — không cần code Play/Stop. `scalingMode: Hierarchy` là điều kiện để `SetAura` scale được cả effect bằng transform.

  ⚠️ **Bẫy hướng — đã trả giá 2026-09-07 (user báo "đang bị lỗi aura", ảnh chụp thấy hai dải xanh khổng lồ dựng đứng quét ngang màn hình).** Cả ba particle system lúc đó (bản chưa tỉa `Waves`) đều phát mesh/billboard **phẳng** (root là Quad `startSize` 6 → đĩa bán kính 3; `Waves` là đĩa mỏng `Cylinder001` extents (0.43, 0.43, 0.05) `startSize` 6 → bán kính ~2.6), và thứ quyết định chúng **nằm ngửa dưới đất hay dựng đứng** là rotation của chính node emitter: phải là **`(270, 0, 0)`** (−90° quanh X) để mặt +Z của mesh quay lên trời. Pack chỉ xoay sẵn node root, để `Waves` và `MagicSparkles` ở identity — nên `Waves` render thành một vòng đứng cao ~6 m và cone sparkle thì phun ngang. Tệ hơn: lúc gắn instance vào `GameplayRoot` mình set `localRotation = identity` cho gọn, **ghi đè luôn 270° của root**, thành ra cái đĩa nền to nhất cũng dựng đứng. Sửa: đặt `(270, 0, 0)` cho mọi node emitter trong prefab, và instance `Aura` trong `GameplayRoot` **giữ đúng rotation của prefab** (đã revert override, `GetPropertyModifications` không còn entry `m_LocalRotation` nào). Đụng vào aura sau này thì đừng zero rotation của nó.

  Lưu ý khi đo lại kích thước: `Renderer.bounds` của particle mesh là **bounding sphere bảo thủ** — một đĩa phẳng 6 đơn vị đang xoay vẫn báo extents ≈ 4.26 ở *cả ba trục*, kể cả trục Y. Đừng dùng con số đó để kết luận effect cao hay phẳng (mình đã hố một lần), hãy đọc `mesh.bounds` của mesh particle + `startSize` + rotation của emitter. **4 asset** đã move từ `ThirdParty/Epic Toon FX` sang `_ZombieWar` bằng `AssetDatabase.MoveAsset` (giữ GUID): 2 material (`slash_circle01_ADD`, `magic_orb2_ADD`) vào `Art/Materials/VFX/` và 2 texture cùng tên vào `Art/Textures/VFX/`. Đợt đầu move 7 asset, nhưng khi xoá `Waves` thì `aura_vertical_ADD.mat` + `gradient_vertical2.png` + `aura_straight.FBX` thành mồ côi nên **đã trả lại `ThirdParty/`** (kèm khôi phục `materialImportMode` về `LegacyImport` như pack gốc) — giữ đúng luật "`_ZombieWar` chỉ chứa asset thật sự dùng". **Không** cần move `Models/Materials/verticalglow02.mat` — `.meta` của FBX không có `externalObjects` remap, nó chỉ tìm material theo tên (`materialSearch: 1`), mà game chỉ dùng *mesh* của FBX (material do ParticleSystemRenderer cấp), nên không có tham chiếu xuyên `ThirdParty/` nào còn lại.
- Nhánh `Abilities/Drone` (local 0): `SentryDroneController` với `_flow`, `_health`, `_playerAim`, `_zombies`, `_projectiles`, `_vfx`, `_audio`, `_enemyMask` = Enemy, `_obstacleMask` = Obstacle, `_anchor` = `Player` (chỉ đọc position), `_body` = con `Body` (local 0, inactive; controller đặt `Body.position` trong world mỗi `LateUpdate`), `_muzzle` = `Body/Muzzle` (0, −0.02, **0.31**). Nhóm `Flight`: `_orbitRadius` 1.4, `_orbitDegreesPerSecond` 60, `_hoverHeight` 1.7, `_hoverAmplitude` 0.12, `_hoverFrequency` 1.2, `_followSmoothTime` 0.2, `_turnSpeedDegrees` 540, `_headingSpeedThreshold` 0.2. **Model drone (thay 2026-09-07 theo yêu cầu user)**: ba primitive dựng tạm (`Hull` Sphere 0.28 + `Glow` Cylinder + `Barrel` Cylinder, material `BombBody`/`WFXM_M_GlowPalet Add`) đã bị xoá, thay bằng một con `Model` — `MeshFilter` + `MeshRenderer` trỏ thẳng vào mesh `Cylinder001` của `Models/Characters/Drone.FBX` và material `Art/Materials/Characters/Drone_Albedo.mat`, **không** instantiate prefab FBX lồng (khỏi phải override material trên instance). Scale **0.15** đưa mesh 3.58 × 3.30 × 3.96 đơn vị về **0.54 × 0.50 × 0.59 m**, cỡ tương đương cụm primitive cũ. `localPosition` = `−mesh.bounds.center × scale` = (0, −0.03, 0) để tâm hình trùng gốc `Body` — gốc `Body` chính là điểm bay mà controller lái, lệch tâm là drone lắc quanh một điểm không phải chính nó.

  - **Hướng mũi**: mesh đối xứng tuyệt đối theo X (đếm đỉnh hai bên khớp từng dải) và lệch theo Z, với gai ăng-ten mảnh duy nhất nằm trên trục giữa ở z ≈ −0.8 và nửa −Z dày/cao hơn → **+Z là mũi**, đúng chiều `LookRotation` mà controller dùng, nên `Model` để rotation identity. Nếu chạy thật thấy drone bay lùi thì sửa bằng cách xoay riêng con `Model` 180° quanh Y — không đụng code, không đụng `Body`.
  - `Muzzle` dời từ z 0.33 → **0.31** = đúng mép mũi (`extents.z × scale` = 0.297 + 0.01).
  - Hai thruster particle (`ThrusterLeft/Right`, `glow1_ADD`, xoay 90° quanh X nên phun xuống) dời từ (±0.22, −0.08, −0.08) sang **(±0.20, −0.20, −0.17)**: vị trí cũ tính cho hull bán kính 0.14, với thân mới rộng ±0.27 thì chúng nằm *lọt trong* mesh và bị che. Toạ độ mới bám hai rotor sau (rotor tâm |x| ≈ 1.34 × scale) và nằm ngay dưới bụng (`−extents.y × scale` + 0.05).
