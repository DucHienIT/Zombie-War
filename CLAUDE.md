...
| `_ZombieWar/Audio/Weapons/` | 6 wav súng (rifle, shotgun; `Pistol_ClipIn_05` làm tiếng tap UI / ném bom / ném molotov; nổ bom đang mượn `AntiMaterialRifle_far_01`; `AssaultCanon_1p_tail` kiêm lửa bén (hazard + molotov) và sóng xung kích; `JackHammer_3p_01` kiêm tiếng sét của CHAIN LIGHTNING; drone bắn chung `AutoGun_3p_01` với rifle; `JackHammer_Reload` không còn ai dùng từ khi bỏ nạp đạn) | PostApocalypseGunsDemo |
...
- `Ground/Skirt` (L1) hoặc `Ground/Dirt` (L2): plane nền lớn, layer Ground, `BoxCollider` thay MeshCollider, mang **`NavMeshModifier ignoreFromBuild`** nên nền ngoài không bao giờ vào NavMesh. `Ground/Arena`: object rỗng layer Ground mang `BoxCollider` 56×56 / 60×60 (mặt trên y = 0) — **nguồn NavMesh duy nhất của sàn**. Ở L1 thêm 16 tile Plane 14 m (`Ground_Outpost`, sân bê tông) chỉ để render, cao hơn skirt 1 cm.
- `Borders`: 4 `BoxCollider` vô hình layer Obstacle ôm sát mép (L1 cao 1.5 m để đạn chạm ngang tầm bao cát, L2 cao 3 m).
- `Fence` (L1): 43 khúc bao cát `Barrier_007` ở ±28.25 (mặt trong trùng tường), cổng phía nam chắn bằng Hummer + 2 hedgehog. `Terrain` (L2): plateau 12×12×3.5 + 4 dốc 22° (số spec giữ nguyên), layer Ground; **plateau mang `NavMeshModifierVolume` Not Walkable trong lòng khối** vì mặt dưới khối từng bake thành sàn walkable rỗng bên trong. `Rocks` (L2): mỏm đá trong sân, vành đá ở 32.8 / 35.5 m (mặt trong không cắt tường 30.5), 7 cụm đồi ở 41–46 m — mỗi boulder là 2 Cube nghiêng ±12–15° đan nhau, chôn `0.5·max(sx,sz)·sin(tilt) + 0.15` để mép nghiêng không hở.
- `Obstacles`: vật cản trong sân (L1 47, L2 26), tâm sân trống bán kính ~4 m. `Dressing`: set piece ngoài rào (tháp canh 4 góc, lều, container radio, trực thăng, xe tăng, vòng hedgehog/bao cát cong/thùng sát ngoài rào để nhìn qua tường luôn có gì đó). `Scatter`: cỏ khô/xương rồng không collider (L1 120; L2 70 ngoài + 18 trong sân, tránh làn dốc).
...
Giết quái nhận XP → đầy thanh thì lên **battle level** (chỉ sống trong một lượt chơi, khác hẳn level tài khoản của Meta progression) → hiện popup chọn 1 trong 3 skill. Hai loại chung một popup: **11 passive** (stat modifier, **mỗi skill đúng một hiệu ứng** — user 2026-09-05 tách 4 thẻ đa hiệu ứng thành 11 thẻ đơn, mỗi `StatId` một thẻ) và **6 active tự kích hoạt** (AUTO GRENADE ném volley theo cooldown, ORBIT BLADES lưỡi cưa xoay quanh người, MOLOTOV ném chai lửa để lại vùng cháy, CHAIN LIGHTNING sét nhảy giữa các zombie, SHOCKWAVE sóng xung kích hất bay đám đông khi bị vây, SENTRY DRONE drone bay bên vai tự bắn — 4 skill sau thêm 2026-09-05 theo duyệt của user). **Không có nút bấm nào** — cơ chế ném bom thủ công đã bỏ hẳn (user 2026-09-05); quả bom chỉ còn tồn tại trong AUTO GRENADE để giữ spec §6. Data ở `Data/Roguelike/` (`RoguelikeSettings.asset` + 17 `Skill_*.asset`), XP mỗi loại quái ở `ZombieDefinitionSO._xpReward` (Walker 10 / Runner 14 / Brute 30 / Giant 90).
...
Còn thiếu / biết trước: nhạc nền (project không có track nào), vỏ đạn văng và decal máu/cháy (spec §12.1, chưa có asset), âm nổ bom đang mượn `AntiMaterialRifle_far_01`, icon súng render từ AssetPreview (`Art/Sprites`). Thư mục `Captures/` (ảnh chụp qua MCP) đã gitignore.
